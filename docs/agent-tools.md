...
- `get_recent_device_events`：读取最近边缘识别事件，例如人体存在、移动侦测、人脸匿名计数、情绪粗分类或安全告警。
- `get_stream_status`：读取实时流配置和在线状态，不直接打开摄像头或绕过 HLS 鉴权。
- `get_media_asset_summary`：只读取媒体元数据，例如类型、大小、空间、事件关联和保留天数，不读取图片或视频内容。
- `plan_adjustment_from_events`：结合边缘事件生成提醒或人工动作建议；控制动作仍必须另走 `control_device`。
- `search_device_docs`：只查询项目内设备协议、统一接入设计、ESP32、STM32 和树莓派示例，返回 MQTT topic、payload、HTTP 入站、入库语义、硬件示例和安全边界摘要。
- `create_automation_rule`：只创建草案；保存必须通过用户确认。当前支持二氧化碳等指标提醒、“晚上 11 点后”时间提醒和边缘事件提醒草案。
...
视觉与媒体类问题会走事件和媒体元数据工具，例如“最近摄像头有没有检测到人”“摄像头在线吗”“根据视觉事件和 CO2 给我调节建议”。智能体默认不读取原始图片、视频或音频；即使大模型已启用，也只能解释工具返回的结构化结果。
