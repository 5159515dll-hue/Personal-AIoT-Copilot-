...
这是一个面向作品集展示的当前版本原型，用来呈现“个人空间智能物联系统”的完整产品形态。系统包含 3D 公开项目页、确定性模拟环境数据、后端接口、前端控制台、工具优先的情感陪伴流程、安全策略检查、审计日志，以及可选的 MQTT / PostgreSQL / TimescaleDB 入站骨架。
...
- 视觉与媒体页支持树莓派边缘识别事件、事件图片/短视频索引和 RTSP 转 HLS 实时流配置；默认不采集，必须先在空间中启用本地处理和媒体策略。
- 带风险等级的设备注册表和可持久化模拟控制；配置数据库时会使用 `device_registry`，否则回退到安全模拟清单。
- 受工具约束的情感陪伴对话，可在模拟数据和数据库遥测之间切换，通过受约束工具读取传感器、设备状态和结构化异常事件作为对话上下文，机器人手势受安全策略门控，关键动作写入审计日志，并可选调用当前大模型生成更自然的共情回复。陪伴侧还支持长期记忆、人格设置和角色管理。
- 中国区模型厂商配置页，预置小米 MiMo 和 Kimi 接口，并可选择陪伴当前模型。
- 只允许低风险模拟动作的策略引擎。
...
- `/` 公开项目页继续开放。
- `/dashboard`、`/spaces`、`/trends`、`/devices`、`/hardware`、`/emotion`、`/models`、`/rules`、`/evaluation`、`/audit` 会跳转到 `/access`。
- `/api/health` 保持公开健康检查。
...
`/models` 页面拆成两个工具：密钥导入工具只负责导入或覆盖厂商密钥；模型切换工具只负责选择陪伴当前使用的厂商、接口和模型。当前预置中国区接口：

- 小米 MiMo：`https://token-plan-cn.xiaomimimo.com/v1`
- 小米 MiMo Anthropic 兼容：`https://token-plan-cn.xiaomimimo.com/anthropic`
- Kimi（月之暗面）：`https://api.moonshot.cn/v1`

情感陪伴仍然坚持工具优先和策略优先：传感器查询、设备状态读取和机器人手势判断先由本地工具和安全策略完成，外部模型只负责在这些结构化结果之后生成更自然的共情回复。提示注入、高风险控制、未知插座和报警器关闭等被策略拒绝的请求不会发送给外部模型。

模型接口密钥按厂商保存在后端本地数据目录。同一厂商第二次导入会覆盖第一次，切换当前模型时不再输入密钥；接口响应只返回脱敏预览。

Kimi K2.6 使用 `max_completion_tokens`，并在 V0 默认关闭 thinking，以避免连接测试或陪伴回复只返回推理字段而没有正文。小米 MiMo Token Plan 同时兼容 `api-key` 和 `Authorization: Bearer` 认证头。
...
`/dashboard`、`/trends`、`/emotion` 和 `/rules` 页面都可以切换到“数据库遥测”。数据库模式会使用已入库的最新传感器读数和历史曲线；如果未配置 `DATABASE_URL` 或暂无数据，控制台会显示明确的不可用或空数据提示。总览页的“遥测链路”卡片会展示数据库连接、样本数、最新入库时间、Timescale 扩展状态、HTTP/MQTT 入站来源分布和最近设备摘要。
...
统一设备接入接口见 `docs/device-connection-interface.md`，传感器消息协议见 `docs/device-protocol.md`，视觉、媒体与实时流协议见 `docs/media-streaming.md`，控制台内也有 `/hardware` 接入帮助页。可执行示例见 `services/mqtt-ingestor/examples/room-node-message.json`、`firmware/esp32-room-node`、`firmware/stm32-room-node` 和 `examples/raspberry-pi-gateway`。ESP32 固件默认只发布遥测，不接收设备控制指令；未接入或读取失败的传感器不会伪造成正常读数。

未来如果接入树莓派摄像头，必须先在 `/spaces` 里把空间、设备、区域和感知能力边界配置清楚，再在 `/devices` 生成设备令牌。当前实现只接收边缘识别结构化结果、事件媒体和受保护 HLS 流；情感陪伴只读取边缘事件和媒体元数据，不做人脸身份库、不保存原始音频、不采集精确 GPS。
...
媒体事件和实时流链路可以单独验收。脚本会创建临时空间、生成设备令牌、上报边缘事件、上传测试图片、创建临时实时流并清理：

```bash
npm run smoke:media
```

核心 API 契约可以单独验收。脚本会验证 `RoomState`、`SensorReading`、`Device`、`AutomationRule`、`PolicyDecision`、`AuditLog` 和模型厂商目录的关键字段，确认小米 MiMo / Kimi 中国区入口存在，并避免公开接口被后续改动破坏：
...
Web 页面路由可以单独验收。脚本会验证公开页、访问口令页、未登录拦截、登录后的总览、趋势、设备、情感陪伴、模型、规则、审计页面，以及 Next.js 同源 `/api/*` 代理鉴权：

```bash
npm run smoke:web
```

部署后可运行服务器烟测，脚本会自动禁用代理环境变量，并验证访问口令、私有 API、结构化异常事件、HTTP 入站、数据库遥测、审计筛选、高风险拒绝和陪伴工具回复：

```bash
npm run smoke:server
```

陪伴动作安全边界可用独立评测脚本验证，覆盖行走/位移手势拒绝、手势意图注入拒绝、未确认手势确认、安全原地手势放行、共情回应附带动作和多轮一致性：

```bash
npm run eval:companion-safety
```

作品集 3 分钟演示链路可用端到端验收脚本验证，覆盖公开项目页、固定口令控制台、情感陪伴模块、模拟环境趋势、设备风险清单和审计追溯：

```bash
npm run acceptance:demo
```

发布前可以运行总验收入口。它会串联后端单元测试、前端类型检查 / lint / 生产构建、API 契约、ESP32 固件协议、Web 页面、MQTT、服务器烟测、情感陪伴安全评测和 3 分钟演示验收：
...
- 数据由确定性模拟器根据时间窗口生成。
- 数据库和 MQTT 已有本地开发骨架；公开演示默认仍使用模拟数据，控制台总览、趋势页、情感陪伴和规则评估可手动切换到数据库遥测。
- 真实设备接入使用 `aiot.v1` 统一协议；设备可注册、心跳、声明遥测能力和批量上报 readings，但不能通过自注册获得控制权限。
- 图片、短视频和实时流走独立媒体通道；情感陪伴只读取边缘事件和媒体元数据，不直接查看原始图片或视频。
- 规则、审计日志、情感陪伴长期记忆和模拟设备状态保存在 `services/api/.local/`；配置数据库后，设备风险元数据会保存在 `device_registry` 表，陪伴记忆情节保留最近 60 天，可在页面行使被遗忘权清空当前角色的全部记忆。
- 自动化规则必须经过用户确认才能创建保存。
...
- 设备 API 和情感陪伴的设备读取工具共用同一个 device adapter；数据库可用时读取 `device_registry`，默认模拟模式读取安全种子清单，低风险模拟控制会更新设备状态并写入审计日志。
