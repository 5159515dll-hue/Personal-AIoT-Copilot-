...
未来如果接入树莓派摄像头，必须先在 `/spaces` 里把空间、设备、区域和感知能力边界配置清楚，再在 `/devices` 生成设备令牌。当前实现只接收边缘识别结构化结果、事件媒体和受保护 HLS 流；不做人脸身份库、不保存原始音频、不采集精确 GPS。
...
- 图片、短视频和实时流走独立媒体通道；智能体只读取边缘事件和媒体元数据，不直接查看原始图片或视频。
